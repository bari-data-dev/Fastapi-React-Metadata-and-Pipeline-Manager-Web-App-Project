...
export const BronzeMappingKowilApi = {
  getAll: async (): Promise<ApiResponse<BronzeMappingKowil[]>> => {
    return httpFetch<ApiResponse<BronzeMappingKowil[]>>(
      `${API_BASE_URL}/bronze-mapping-kowil`,
      { method: "GET" }
    );
  },

  getById: async (mappingId: number): Promise<ApiResponse<BronzeMappingKowil>> => {
    if (!mappingId) throw new Error("mappingId is required");
    return httpFetch<ApiResponse<BronzeMappingKowil>>(
      `${API_BASE_URL}/bronze-mapping-kowil/${mappingId}`,
      { method: "GET" }
    );
  },

  create: async (
    payload: Omit<BronzeMappingKowil, "mapping_id" | "dwh_loaded_at">
  ): Promise<ApiResponse<BronzeMappingKowil>> => {
    if (!payload) throw new Error("payload is required");

    // minimal validation: at least one field filled
    const hasAny = Object.values(payload).some(
      (v) => v !== undefined && v !== null && String(v).trim() !== ""
    );
    if (!hasAny) throw new Error("At least one field must be provided");
  ...
  batchAdd: async (
    rows: Array<Omit<BronzeMappingKowil, "mapping_id" | "dwh_loaded_at">>
  ): Promise<ApiResponse<BronzeMappingKowil[]>> => {
    ...
    const invalid = rows.some((r) => {
      const hasAny = Object.values(r).some(
        (v) => v !== undefined && v !== null && String(v).trim() !== ""
      );
      return !hasAny;
    });
    ...
    if (invalid) {
      throw new Error("Each row must include at least one non-empty field");
    }

    return httpFetch<ApiResponse<BronzeMappingKowil[]>>(
      `${API_BASE_URL}/bronze-mapping-kowil/batch-add`,
      {
        method: "POST",
  ...
  update: async (
    mappingId: number,
    payload: Partial<Omit<BronzeMappingKowil, "mapping_id" | "dwh_loaded_at">>
  ): Promise<ApiResponse<BronzeMappingKowil>> => {
    if (!mappingId) throw new Error("mappingId is required");

    // minimal: harus ada field yang diupdate
    const hasAny = payload && Object.keys(payload).length > 0;
    if (!hasAny) throw new Error("No fields provided to update");
