...
type ArtbstRecord = {
  ...
  artcode?: string | null;
  oms30_0?: string | null;
  u_konversi?: number | null;
  verkp_verp?: number | null;
...
function makeInsertDraft(values: Record<string, string> = EMPTY_INSERT): InsertDraft {
  return {
...
export default function ArtbstPage() {
  const { user } = useAuth();
  const [data, setData] = useState<ArtbstPageData | null>(null);
  ...
  const load = async () => {
    ...
    setError("");
    try {
      const query = buildQuery({ page, pageSize, filters: appliedFilters, sortBy, sortDir });
      const response = await appFetch<ArtbstPageData>(`/artbst?${query.toString()}`);
      setData(response.data);
      if (response.data.page !== page) setPage(response.data.page);
      setDraftRows({});
    } catch (err) {
      setError(err instanceof Error ? err.message : "Gagal mengambil ARTBST");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    void load();
  }, [allowed, page, pageSize, JSON.stringify(appliedFilters), sortBy, sortDir]);

  useEffect(() => {
    if (!allowed || !valuePickerField) return;
    let cancelled = false;
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        const response = await appFetch<DistinctValue[]>(
          `/artbst/values/${encodeURIComponent(valuePickerField)}?${query.toString()}`
        );
        if (!cancelled) {
  ...
  }, [allowed, valuePickerField, valuePickerSearch, JSON.stringify(appliedFilters)]);
  ...
  const columns = DISPLAY_COLUMN_ORDER.map((name) => columnMap.get(name)).filter(
    (column): column is ArtbstColumn => Boolean(column)
  );
  ...
  const totalTableWidth =
    columns.reduce((total, column) => total + getColumnWidth(column.name), 0) + ACTION_WIDTH;
  ...
  const rowId = (row: ArtbstRecord) => String(row.id);
  const getCellValue = (row: ArtbstRecord, field: string) => {
    const draft = draftRows[rowId(row)];
    return draft && field in draft ? draft[field] : row[field as keyof ArtbstRecord];
  };

  const setCellValue = (row: ArtbstRecord, field: string, value: unknown) => {
  ...
  const setInsertCellValue = (localId: string, field: string, value: string) => {
    setSuccessMessage("");
  ...
  const getChangedValues = (row: ArtbstRecord) => {
    const draft = draftRows[rowId(row)] || {};
    const changed: Record<string, unknown> = {};
    Object.entries(draft).forEach(([field, value]) => {
      const original = row[field as keyof ArtbstRecord];
  ...
  const pendingUpdates: BatchUpdateItem[] = (data?.items || [])
  ...
  const totalChangedFields =
    pendingUpdates.reduce((total, item) => total + Object.keys(item.values).length, 0) +
    insertRows.reduce(
      (total, row) => total + Object.values(row.values).filter((value) => value !== "").length,
      0
    );

  const cancelRow = (row: ArtbstRecord) => {
    setDraftRows((current) => {
      const next = { ...current };
  ...
  const duplicateExistingRow = (row: ArtbstRecord) => {
    const values = EDITABLE_FIELDS.reduce<Record<string, string>>((result, field) => {
  ...
  const removeInsertRow = (localId: string) => {
    setInsertRows((current) => current.filter((row) => row.localId !== localId));
  };

  const togglePendingDelete = (row: ArtbstRecord) => {
  ...
  const saveAllChanges = async () => {
    ...
    setSuccessMessage("");
    try {
      const response = await appFetch<SaveResult>("/artbst/save", {
        method: "POST",
        body: JSON.stringify({
      ...
      setError(err instanceof Error ? err.message : "Save ARTBST gagal");
    } finally {
  ...
  const applySelectedValues = () => {
    ...
    if (selectedValues.length) next[valuePickerField] = `__IN__:${JSON.stringify(selectedValues)}`;
    else delete next[valuePickerField];
  ...
  const renderEditableCell = (
    ...
    const changed =
      ...
      String(normalizedValue(value) ?? "") !==
        String(normalizedValue(row[column.name as keyof ArtbstRecord]) ?? "");
    ...
    if (!column.editable) {
      return (
        ...
        >
          ...
          {column.name === "id" && !isPendingDelete && Object.keys(getChangedValues(row)).length > 0 && (
            <Button
              variant="outline"
              size="sm"
  ...
  const renderInsertCell = (row: InsertDraft, column: ArtbstColumn) => {
    if (column.name === "id") {
      return (
        <td
          key={column.name}
    ...
    if (!column.editable) {
      const autoValue =
        column.name === "dwh_created_by" || column.name === "dwh_created_at" ? "AUTO" : "-";
      return (
        <td
          key={column.name}
    ...
    return (
      ...
      >
        <Input
          ...
          maxLength={column.max_length || undefined}
          onChange={(event) => setInsertCellValue(row.localId, column.name, event.target.value)}
        />
      </td>
    );
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-xl font-bold sm:text-2xl">ARTBST</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            CRUD product master pada bronze_so.ARTBST.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button onClick={requestBatchSave} disabled={!dirtyRowCount || batchSaving}>
            Save Changes{dirtyRowCount ? ` (${dirtyRowCount})` : ""}
          </Button>
          <Button variant="outline" onClick={discardAll} disabled={!dirtyRowCount || batchSaving}>
            Discard All
          </Button>
      ...
      {dirtyRowCount > 0 && (
        <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-amber-950 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-100">
          ...
          {insertRows.length ? `, ${insertRows.length} row baru` : ""}
          {pendingUpdates.length ? `, ${pendingUpdates.length} row diedit` : ""}
          {pendingDeleteIds.length ? `, ${pendingDeleteIds.length} row akan dihapus` : ""}
          {totalChangedFields ? `, ${totalChangedFields} field terisi/berubah.` : "."}
        </div>
      )}
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-muted-foreground">
                {activeFilterCount ? `${activeFilterCount} filter aktif` : "Tidak ada filter aktif"}
              </span>
            </div>
          ...
          <div className="max-h-[62dvh] w-full touch-auto overflow-auto overscroll-contain rounded-md border sm:max-h-[68vh]">
            ...
            >
              ...
              <thead className="sticky top-0 z-20 bg-background shadow-sm">
                <tr>
                  {columns.map((column) => {
                    const chosenCount = parseSelectedValues(appliedFilters[column.name]).length;
                    return (
                      <th
                        key={column.name}
                        className={cn(
                          "relative border-b p-2 text-left",
                          column.name === "id" && "sticky left-0 z-30 bg-background"
                        )}
                        style={{ width: getColumnWidth(column.name) }}
                      >
                        ...
                        <div
                          ...
                          onMouseDown={(event) => beginColumnResize(event, column.name)}
                          onDoubleClick={(event) => {
                            event.preventDefault();
                            event.stopPropagation();
              ...
              <tbody>
                {insertRows.map((row) => (
                  <tr key={row.localId} className="bg-amber-100 dark:bg-amber-950/60">
                    ...
                    <td className="sticky right-0 border-b border-amber-300 bg-amber-100 p-2 align-middle text-center dark:border-amber-800 dark:bg-amber-950/60">
                      <div className="flex items-center justify-center gap-1">
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-8 w-8 border-emerald-500 bg-emerald-100 p-0 text-emerald-700 hover:bg-emerald-200 hover:text-emerald-800 dark:border-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300"
                ...
                {data?.items.length ? (
                  data.items.map((row) => {
                    ...
                    return (
                      ...
                      >
                        {columns.map((column) => renderEditableCell(row, column, isPendingDelete))}
                        <td
                          className={cn(
                        ...
                        >
                          <div className="flex items-center justify-center gap-1 no-underline">
                            <Button
                              ...
                              disabled={batchSaving || isPendingDelete}
                            >
                              <Copy className="h-3.5 w-3.5" />
                            </Button>
                            <Button
                              ...
                              className={cn(
                                ...
                                isPendingDelete && "border-destructive text-destructive hover:bg-destructive/10"
                              )}
                              title={isPendingDelete ? `Batalkan delete ID ${row.id}` : `Delete ID ${row.id}`}
                  ...
                  <tr>
                    <td colSpan={columns.length + 1} className="p-8 text-center">Memuat ARTBST...</td>
                  </tr>
                ) : (
                  <tr>
      ...
      {batchConfirmOpen && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/50 p-4">
          <Card className="w-full max-w-lg">
            <CardHeader><CardTitle>Confirm Save</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <p>Simpan <strong>{dirtyRowCount}</strong> row perubahan ke ARTBST?</p>
              ...
              <CardContent className="flex min-h-0 flex-1 flex-col p-0">
                <div className="space-y-3 border-b p-4 sm:px-6">
                  ...
                  <div className="flex items-center justify-between rounded-md border bg-muted/30 px-3 py-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setSelectedValues([])}>
                      Reset pilihan
                    </Button>
                  </div>
