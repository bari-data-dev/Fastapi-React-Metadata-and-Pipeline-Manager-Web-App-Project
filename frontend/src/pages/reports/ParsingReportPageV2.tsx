...
export default function ParsingReportPageV2() {
  ...
  useEffect(() => {
    ...
    if (tab === "effective") {
      parsingReportApi
        ...
        .catch((err) => {
          if (!cancelled) {
            setError(err instanceof Error ? err.message : "Gagal mengambil effective result");
          }
        })
        .finally(() => {
      ...
      parsingReportApi
        ...
        .catch((err) => {
          if (!cancelled) {
            setError(err instanceof Error ? err.message : "Gagal mengambil activity history");
          }
        })
        .finally(() => {
          if (!cancelled) setLoadingTable(false);
    ...
    <div className="min-w-0 space-y-4 p-3 text-sm sm:p-6">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Effective Result membandingkan data saat ini dengan baseline awal. Activity
          History menampilkan seluruh aktivitas before-after, termasuk revert.
        </p>
      </div>
            ...
            <thead className="bg-muted/50">
              <tr>
                {[
                  ...
                  <th key={header} className="border-b p-3 text-left text-xs font-semibold">
                    {header}
                  </th>
                ))}
              </tr>
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <label className="space-y-1">
            <span className="text-xs font-semibold text-muted-foreground">ODISTS ID</span>
            <Input
              inputMode="numeric"
              value={odistIdInput}
          ...
          <label className="space-y-1">
            <span className="text-xs font-semibold text-muted-foreground">REVERT STATE</span>
            <select
              className="h-10 w-full rounded-md border bg-background px-3"
              value={activeRevertState}
              onChange={(event) => {
                if (tab === "effective") setEffectiveRevertState(event.target.value);
                else setHistoryRevertState(event.target.value);
                setPage(1);
              }}
            >
          ...
          <div className="flex items-end gap-2">
            ...
            <Button className="flex-1" variant="outline" onClick={resetTableFilters}>
              Reset
            </Button>
          </div>
        </CardContent>
                ...
                <tbody>
                  {loadingTable ? (
                    ...
                    effective.items.map((item) => (
                      <tr
                        key={`${item.odist_id}-${item.member_user_id ?? "untracked"}`}
                      ...
                      >
                        ...
                        <td className="border-b p-3">
                          {item.owned_revision_fields.length
                            ? item.owned_revision_fields.map(displayField).join(", ")
                            : "-"}
                        </td>
                        <td
                        ...
                        <td className="whitespace-nowrap border-b p-3">{formatDate(item.last_edited_at)}</td>
                        <td className="border-b p-3 tabular-nums">{item.total_actions}</td>
                        <td className="border-b p-3">
                          {item.is_untracked ? <StatusBadge value="UNTRACKED" /> : "TRACKED"}
                        </td>
                      </tr>
                    ))
                  ) : (
          ...
          <div className="flex flex-col gap-3 px-3 pb-3 sm:flex-row sm:items-center sm:justify-between sm:px-0 sm:pb-0">
            ...
            <div className="flex items-center justify-between gap-2 sm:justify-end">
              ...
              <span className="whitespace-nowrap">
                Page {activePage?.page ?? page} / {activePage?.total_pages ?? 1}
              </span>
              <Button
                variant="outline"
                size="sm"
